...
export default function ChannelPage() {
  ...
  useEffect(() => {
    ...
    socket.on("channelUpdated", (data: { channelId: string; members: number }) => {
      if (data.channelId === channelId) setMembers(data.members);
    });
  ...
  useEffect(() => {
    ...
    if (!peerConnectionRef.current) createPeerConnection(!isCreator);
  ...
  const createPeerConnection = (isOfferer: boolean) => {
    ...
    pc.onicecandidate = (event) => {
      if (event.candidate) socket.emit("send-ice-candidate", { channelId, candidate: event.candidate });
    };

    pc.oniceconnectionstatechange = () => {
      console.log(`ICE Connection State: ${pc.iceConnectionState}`);
      if (pc.iceConnectionState === "connected") console.log("Peers are fully connected!");
    };
    ...
    if (isOfferer) {
      ...
      pc.createOffer()
        ...
        .then(() => socket.emit("send-offer", { channelId, offer: pc.localDescription }))
  ...
  const handleOffer = async (data: { offer: RTCSessionDescriptionInit; senderId: string }) => {
    if (!peerConnectionRef.current) createPeerConnection(false);
    ...
    dc.onmessage = (event) => {
      ...
      if (typeof data === "string") {
        ...
        if (message.type === "metadata") {
          ...
          setIsTransferring(true); // Sync transfer state
          ...
          setIsTransferring(false);
          console.log(`File ${fileId} cancelled by peer and removed`);
        } else if (message.type === "transferStart") {
          setIsTransferring(true);
          console.log("Transfer started by peer");
        } else if (message.type === "transferEnd") {
          setIsTransferring(false);
          console.log("Transfer ended by peer");
        }
        ...
        if (receivedCount === fileData.total) {
          ...
          if (blob.size === fileData.size) {
            ...
            setTimeout(() => {
              setFiles((prev) => prev.filter((f) => f.id !== fileId));
              dc.send(JSON.stringify({ type: "transferEnd" }));
              setIsTransferring(false);
            }, 5000);
          } else {
  ...
  const sendFile = async (id: string) => {
    ...
    if (
      ...
      setIsTransferring(true);
      dataChannelRef.current.send(JSON.stringify({ type: "transferStart" }));
      ...
      const sendNextChunk = () => {
        if (abortTransferRef.current[fileId]) {
          ...
          const cancelMessage = { type: "cancel", fileId };
        ...
        if (offset < fileToSend.size) {
          ...
          if (bufferedAmount > CHUNK_SIZE * 4) {
            const waitTime = bufferedAmount > CHUNK_SIZE * 8 ? 200 : 50;
          ...
          setTimeout(() => {
            ...
            dataChannelRef.current!.send(JSON.stringify({ type: "transferEnd" }));
            setIsTransferring(false);
          }, 5000);
        }
      };
  ...
  const cancelFile = (id: string) => {
    ...
    if (file && (file.status === "transferring" || file.status === "received") && dataChannelRef.current) {
      abortTransferRef.current[id] = true;
      const cancelMessage = { type: "cancel", fileId: id };
      dataChannelRef.current.send(JSON.stringify(cancelMessage));
      console.log("Sent cancel message from cancelFile:", cancelMessage);
    }
    setFiles((prev) => prev.filter((f) => f.id !== id));
    setIsTransferring(false);
  };
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "transferring": return <Upload className="h-4 w-4 text-blue-500 animate-pulse" />;
